// Thin client for the SiliconFlow image + video generation APIs.
//
// Requests go through the Vite dev proxy (`/api/siliconflow` -> the SiliconFlow
// API host) so the browser never talks to the API host directly and CORS is a
// non-issue. See vite.config.js.

const BASE = '/api/siliconflow'
 ...
 *
 * @param {object} opts
 * @param {string} opts.apiKey       SiliconFlow API key (Bearer token).
 * @param {string} opts.model        Model id, e.g. "Kwai-Kolors/Kolors".
...
export async function generateVideo(opts) {
  ...
  throw new Error(
    `Timed out after ${Math.round(timeoutMs / 60000)} min. The job (${requestId}) may still finish on SiliconFlow — check your dashboard or try again.`,
  )
}
